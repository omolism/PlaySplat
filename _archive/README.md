...
### PlaySplat_PC.spz.v4-incompatible-with-spark-0.1.10
...
gracefully falls back to `PlaySplat_PC.splat`.
...
1. **Regenerate as SPZ v3** — easiest path is `node scripts/generate-spz-v3.mjs`,
   ...
   `public/PlaySplat_Mobile.spz`, which `pickSplatUrl()` then picks
